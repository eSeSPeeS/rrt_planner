  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  if (goalChanged(gx, gy)) {
    ...
    // Wyczyść plik statystyk przy nowym celu
    std::ofstream f("/home/pawel/rrt_stats.txt", std::ios::trunc);
    f << "# Nowy cel: (" << gx << ", " << gy << ")\n";
  }
  ...
  // ── Pomiar czasu planowania ───────────────────────────────────────────────
  // Czas startuje od początku planowania (reset przy nowym celu).
  // Przy każdym znalezieniu/poprawieniu ścieżki logujemy czas i długość.
  static auto planning_start = std::chrono::steady_clock::now();
  if (tree_.size() == 1) {
    // Właśnie zresetowaliśmy drzewo – startujemy timer od nowa
  ...
  for (int i = 0; i < max_iterations_; ++i) {
    ...
    for (int ni : X_near) {
      ...
      if (c < tree_[ni].cost && obstacleFree(nx, ny, tree_[ni].x, tree_[ni].y)) {
        ...
        tree_[ni].cost       = c;
        double old_goal_cost = best_goal_cost_;
        ...
        // Jeśli rewiring poprawił koszt celu – zaloguj
        if (best_goal_cost_ < old_goal_cost) {
          auto now_tp = std::chrono::steady_clock::now();
          double elapsed_ms = std::chrono::duration<double, std::milli>(
            now_tp - planning_start).count();
          fprintf(stderr,
            "[RRT* STATS] iter=%d  czas=%.1f ms  dlugosc=%.3f m  wezly=%zu  (rewire)\n",
            i+1, elapsed_ms, best_goal_cost_, tree_.size());
          fflush(stderr);
          std::ofstream f("/home/pawel/rrt_stats.txt", std::ios::app);
          if (f.is_open()) {
            f << "iter=" << (i+1)
              << "  czas=" << elapsed_ms << " ms"
              << "  dlugosc=" << best_goal_cost_ << " m"
              << "  wezly=" << tree_.size()
              << "  (rewire)\n";
            f.flush();
          }
    ...
    if (goalReached(nx, ny, gx, gy) && c_min < best_goal_cost_) {
      ...
      // ── Pomiar czasu i długości ścieżki ──────────────────────────────────
      auto now_tp = std::chrono::steady_clock::now();
      double elapsed_ms = std::chrono::duration<double, std::milli>(
        now_tp - planning_start).count();
      double path_length = best_goal_cost_;

      // Zawsze widoczny print (niezależnie od poziomu logów ROS2)
      fprintf(stderr,
        "[RRT* STATS] iter=%d  czas=%.1f ms  dlugosc=%.3f m  wezly=%zu\n",
        i+1, elapsed_ms, path_length, tree_.size());
      fflush(stderr);
      ...
      if (f.is_open()) {
        f << "iter=" << (i+1)
          << "  czas=" << elapsed_ms << " ms"
          << "  dlugosc=" << path_length << " m"
          << "  wezly=" << tree_.size()
          << "\n";
        f.flush();
      } else {
        fprintf(stderr, "[RRT* STATS] BLAD: nie mozna otworzyc pliku!\n");
      }
      // ─────────────────────────────────────────────────────────────────────
...
bool RRTPlanner::obstacleFree(double x0,double y0,double x1,double y1) const
{
  ...
  for (int s=0; s<=steps; ++s) {
    ...
    if (costmap_->getCost(mx,my) >= 180) { return false; }
  }
  return true;
}
